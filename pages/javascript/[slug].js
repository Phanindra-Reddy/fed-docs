...
import { getJsTopic, getJsTopics } from "../../services";
...
function JsTopic({ data }) {
  ...
  return (
    <>
      <HeadComp title="JavaScript" description={data?.topicName} />
      <ToastContainer />
      <div className="flex flex-col h-screen">
        <div className="flex-1">
          <div className="flex flex-row">
            ...
            <div className="w-full lg:w-3/4 p-5 md:px-10 dark:bg-gray-700 dark:text-white">
              <RichText
                ...
                renderers={{
                  ...
                  code: ({ children }) => (
                    <code className="bg-gray-100 px-1 rounded-md border border-gray-300 text-black text-sm">
                      {children}
                    </code>
                  ),
                  code_block: ({ children }) => {
...
export async function getStaticProps({ params }) {
  const data = await getJsTopic(params.slug);
...
export async function getStaticPaths() {
  const topics = await getJsTopics();
